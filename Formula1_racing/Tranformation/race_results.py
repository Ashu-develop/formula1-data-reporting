...
constructors_df = spark.read.parquet(f"{processed_folder_path}/constructor").withColumnRenamed("name","team")
...
# MAGIC %md
# MAGIC **Join rsult to all other dataframes**

# COMMAND ----------

race_results_df = results_df.join(race_circuits_df, results_df.race_id==race_circuits_df.race_id).join(drivers_df,results_df.driver_id==drivers_df.driver_id).join(constructors_df,results_df.constructor_id==constructors_df.constructor_id)
